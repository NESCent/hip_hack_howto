...
<table>
<tr><th>Tangible Outcome</th><th>Measure(s)</th></tr>
<tr>
 <td>Code
 </td>
 <td>
...
### Scoping with flexibility and limits (Shann)

How a hackathon is scoped should be directly led by its specific objective and goals as there most likely will be significant tradeoffs. A well-defined scope can ease the decision making process regarding those tradeoffs as well as manage the expectations of potential participants.  For example, NESCent's Phylotastic II hackathon had a very open-scope with the greatest talent diversity. However, it also produced fewer tangible products which may be due to its loose structure. In contrast, Phylotastic I was open-scoped with far less talent diversity, but had strong tangible results. As a result, there may exist a tension between the goals of producing tangible products and increasing talent diversity. We have found that having two disparate goals of producing large amounts of useful code and community building is challenging, particularly during application evaluation and facilitation

A loosely-defined scope also leads to people to working increasing on their own rather than in groups, such as in the VoCamp hackathon.
